...
const rows=[{input:{evidenceHash:judgment.status==="success"?judgment.evidenceHash:"none"},prediction:judgment,provenance:{containerGroup:"blade-web",lineageGroup:"blade-web",synthetic:true,samplingReasons:["synthetic"]},label:{evidenceHash:"synthetic",purchaseBehaviorPreserved:"pass" as const,reviewer:"synthetic",reviewedAt:"not-human"},route}]; const split=splitByGroup(rows);assertGroupDisjoint(split); console.log(JSON.stringify({route,actualLegacyAction:after.combinedScore>before.combinedScore?"improved":"reverted",datasetJsonl:exportJsonl(rows),report:replayReport(rows)},null,2));
