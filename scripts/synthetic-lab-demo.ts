...
const summary = {
  ...
  splitCoverage: Object.fromEntries(Object.entries(split).map(([key, value]) => [key, { observations: value.length, topologyGroups: [...new Set(value.map(row=>row.provenance.topologyGroup).filter(Boolean))], lineageGroups: [...new Set(value.map(row=>row.provenance.lineageGroup))] }])),
