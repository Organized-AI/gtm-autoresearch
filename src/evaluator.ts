...
function scoreParameterCompleteness(tags: GtmTag[]): DimensionScore {
  ...
  for (const tag of tags) {
    const reqs = (Object.hasOwn(TAG_PARAM_REQUIREMENTS, tag.type) ? TAG_PARAM_REQUIREMENTS[tag.type] : undefined);
    if (!reqs) continue;
...
function scoreNamingConventions(
  ...
  for (const v of variables) {
    const expectedPrefix = (Object.hasOwn(VAR_PREFIX_MAP, v.type) ? VAR_PREFIX_MAP[v.type] : undefined);
...
function scoreCapiCoverage(
  ...
  for (const event of metaEvents) {
    ...
    const pattern = (Object.hasOwn(META_EVENT_TO_TAG_PATTERN, event.event) ? META_EVENT_TO_TAG_PATTERN[event.event] : undefined);
...
function scoreGoogleAdsAlignment(
  ...
  for (const action of actions) {
    ...
    const pattern =
      (Object.hasOwn(GOOGLE_ADS_CATEGORY_TO_TAG_PATTERN, action.category) ? GOOGLE_ADS_CATEGORY_TO_TAG_PATTERN[action.category] : undefined) ??
    ...
    if (matchingTag) {
      ...
      issues.push({
        ...
        severity,
        entity: action.name,
        message: `Google Ads conversion "${action.name}" (${action.category}, ${action.conversion_count_30d} conversions, $${action.conversion_value_30d.toLocaleString("en-US")}) has no matching GTM tag`,
      });
    }
  }
  ...
  for (const event of events) {
    ...
    if (!pattern) {
      ...
      issues.push({
        ...
        severity: "info",
        entity: event.event,
        message: `Meta event "${event.event}" has no known GTM tag pattern (${event.count_7d_click} events, $${event.value_7d_click.toLocaleString("en-US")})`,
      });
      continue;
    }
    ...
    if (hasTag) {
      ...
      issues.push({
        ...
        severity,
        entity: event.event,
        message: `Meta fires "${event.event}" (${event.count_7d_click} events, $${event.value_7d_click.toLocaleString("en-US")} value) but no GTM Meta tag found`,
      });
    }
  }
