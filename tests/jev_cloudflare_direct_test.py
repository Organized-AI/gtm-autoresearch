...
class DirectCloudflareTest(unittest.TestCase):
 def test_completed_gateway_wrapper_matches_observed_live_shape(self):
  result={"model":"jev-1.13.0","answers":{"trackingBehaviorPreserved":{"type":"choice","choice":"pass","confidence":.28,"probabilities":{"pass":.52,"fail":.44,"insufficient":.04}}},"usage":{"input_tokens":7637,"output_tokens":49}}
  wrapper={"result":{"state":"Completed","result":result,"gatewayMetadata":{"keySource":"Unified"}},"success":True,"errors":[],"messages":[]}
  self.assertEqual(module.unwrap(wrapper),result)
  for state in ("Pending","Failed",None):
   wrapper["result"]["state"]=state
   with self.assertRaisesRegex(ValueError,"not completed"): module.unwrap(wrapper)
  wrapper["result"]["state"]="Completed"; wrapper["errors"]=[{"code":1000}]
  with self.assertRaisesRegex(ValueError,"API error"): module.unwrap(wrapper)
