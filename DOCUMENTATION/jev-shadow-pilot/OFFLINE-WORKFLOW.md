...
Version 2 manifests require each case to declare `lineage_group`, `container_group`, `topology_group`, and `split` (`train`, `validation`, or `holdout`). Connected components span any shared container, lineage, or topology group. A topology can never cross a split; conflicting or partial declared components are rejected. These controls protect replay partitioning only. Coverage across topologies that share simulator recipes does not establish generalization.
