...
The entire corpus shares one lineage and topology, so all observations stay in one split. Empty validation/holdout files are intentional and are reported as insufficient evaluation coverage. No prediction accuracy, probability calibration, causal inference, or readiness for automatic acceptance is established by this run.
